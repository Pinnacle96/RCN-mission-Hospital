  ...
  <div class="relative z-10 max-w-7xl mx-auto px-4 py-20">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-lg text-white/90">Preparing new ways to serve and reach more people.</p>
      </div>
    </div>
  </div>
...
<?php
  try {
    $stmt = db()->query('SELECT title, description, start_date, end_date FROM future_programs ORDER BY id DESC');
    ...
    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
      <?php foreach ($programs as $p): ?>
        <article class="bg-white rounded-2xl shadow border border-gray-100 overflow-hidden hover:shadow-md transition">
          <div class="p-6">
            <h3 class="text-xl font-semibold text-gray-900 mb-2"><?php echo esc_html($p['title']); ?></h3>
            <?php if (!empty($p['start_date']) || !empty($p['end_date'])): ?>
              <p class="text-sm text-gray-500 mb-2">
                <?php echo !empty($p['start_date']) ? esc_html(date('M j, Y', strtotime($p['start_date']))) : ''; ?>
                <?php if (!empty($p['end_date'])): ?> - <?php echo esc_html(date('M j, Y', strtotime($p['end_date']))); ?><?php endif; ?>
              </p>
            <?php endif; ?>
            <p class="text-midnight/80"><?php echo esc_html($p['description']); ?></p>
          </div>
        </article>
      <?php endforeach; ?>
    </div>
  <?php else: ?>
    <div class="text-center py-16">
      <div class="max-w-md mx-auto">
        <h3 class="text-2xl font-semibold text-gray-900 mb-3">No Future Programs Published</h3>
        <p class="text-gray-500 mb-6">We’re prayerfully preparing new initiatives. Check back soon for updates.</p>
