<?php
// Initialize a Paystack transaction for a one-time payment
// Expects POST: email, amount (in NGN)
...
if ($amountNgn <= 0) {
    log_error('paystack_payment', 'Invalid amount', ['amount' => $amountNgn]);
    respond(false, ['error' => 'Valid amount (NGN) is required']);
}

// Build payload for Paystack initialize (amount in kobo)
$reference = 'rcn_once_' . time() . '_' . bin2hex(random_bytes(4));
$payload = [
    'email' => $email,
    'amount' => $amountNgn * 100,
    'reference' => $reference,
];
log_info('paystack_payment', 'Init once request', ['email' => $email, 'amount_ngn' => $amountNgn, 'reference' => $reference]);
...
$resp = curl_exec($ch);
if ($resp === false) {
    log_error('paystack_payment', 'cURL error on init_once', ['error' => curl_error($ch)]);
...
if ($json['status'] === true && isset($json['data']['authorization_url'])) {
    log_info('paystack_payment', 'Init once success', ['authorization_url' => $json['data']['authorization_url'], 'reference' => $reference]);
    respond(true, [
...
log_error('paystack_payment', 'Init once failed', ['message' => $json['message'] ?? 'Unable to initialize transaction', 'reference' => $reference]);
